...
import { useUpdateCrossConversionMutation, type CrossConversionResponse } from '../../services/crossConversionApi'
import { BRANDS, getBrandLabel, getBrandColor, type Brand } from '../../constants/brandConstants'

interface Props {
    item: CrossConversionResponse
    onClose: () => void
    onSuccess?: () => void
}

export default function EditCrossConversionModal({ item, onClose, onSuccess }: Props) {
    const [updateCrossConversion, { isLoading }] = useUpdateCrossConversionMutation()
    const { data: customers = [] } = useListCustomersQuery()

    const [selectedCustomerId, setSelectedCustomerId] = useState(item.customerId)
    const [selectedOrderId, setSelectedOrderId] = useState(item.orderId)
    const [sourceBrand, setSourceBrand] = useState<Brand>(item.sourceBrand as Brand)
    const [targetBrand, setTargetBrand] = useState<Brand>(item.targetBrand as Brand)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error('Failed to update cross conversion:', error)
            const errorMessage = error?.data?.message || error?.message || 'Güncelleme sırasında bir hata oluştu'
            alert(errorMessage)
        }
    }
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white border border-amber-200 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between p-6 border-b border-amber-200">
                    <h2 className="text-2xl font-bold text-amber-900">Çapraz Dönüştürme Güncelle</h2>
