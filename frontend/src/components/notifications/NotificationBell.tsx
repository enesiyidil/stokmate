...
export default function NotificationBell() {
    ...
    // Check if session is expired to stop polling
    ...
    const { data: notifications = [], refetch } = useGetAllNotificationsQuery(undefined, {
        pollingInterval: sessionExpired ? 0 : 30000, // Stop polling when session expired
        skip: sessionExpired, // Skip query entirely when session expired
    })
    const { data: countData } = useGetUnreadCountQuery(undefined, {
        pollingInterval: sessionExpired ? 0 : 30000,
        skip: sessionExpired,
    })
                            ...
                            notifications.map((notification) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-start gap-3 p-4 hover:bg-gray-50 transition-all border-b border-gray-100 text-left relative
                                            ${!notification.isRead ? 'bg-amber-50/60' : 'bg-white opacity-80'}
                                        `}
                                    >
                                        {!notification.isRead && (
                                            <span className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500"></span>
                                        )}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-sm mb-0.5 ${!notification.isRead ? 'font-bold text-gray-900' : 'font-medium text-gray-600'}`}>
                                                {notification.title}
                                            </p>
                                        ...
                                        {!notification.isRead && (
                                            <div className="w-2.5 h-2.5 bg-amber-500 rounded-full flex-shrink-0 mt-2 shadow-sm animate-pulse" />
                                        )}
                                    </button>
                                )
                            })
